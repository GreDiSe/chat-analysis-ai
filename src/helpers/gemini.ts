...
interface ChatAnalysis {
  ...
  top_3_topics: string[]; // most discussed topics 2-4 words (keywords)
  ...
  most_used_love_words: { phrase: string, count: number }[]; // love words used (word + count)
  most_romantic: Name | null; // most romantic speaker (name)
  laugh_together_count: number; // shared laughs (count)
  funniest_message: string[] | null; // most funny message (text)
  favorite_love_emoji: { emoji: string, count: number }[] | null; // most used emoji (character)
  relationship_direction: string; // relationship status/trajectory (text +- 10 words)
}
...
export interface GeneralChatAnalysis {
  ...
  most_used_phrase: Record<Name, string>;                   // Most texted phrase per person (text 3-4 words)
  ...
  general_summary: string;                                  // Plaintext overview of chat (text 10-12 words)
  personal_summary: Record<Name, string>;                   // Summary for each individual (text 5-7 words)
}
  ...
  try {
    ...
    if (parsed && userId) {
      try {
        ...
        console.warn('Failed to append AI response to Firestore', e);
      }
    }
